...
const create = async (req, res) => {
  ...
  try {
    const query = 'INSERT INTO pagos_servicios (servicio, referencia, monto, fecha_pago, metodo) VALUES (?, ?, ?, ?, ?)';
    const [result] = await db.query(query, [servicio, referencia, monto, fecha_pago, metodo]);
...
const update = async (req, res) => {
  const { servicio, referencia, monto, fecha_pago, metodo } = req.body;
  if (!servicio || !referencia || !monto || !fecha_pago || !metodo) {
    return res.status(400).json({ error: 'Todos los campos son obligatorios' });
  }
  try {
    const query = 'UPDATE pagos_servicios SET servicio=?, referencia=?, monto=?, fecha_pago=?, metodo=? WHERE id=?';
    const [result] = await db.query(query, [servicio, referencia, monto, fecha_pago, metodo, req.params.id]);
